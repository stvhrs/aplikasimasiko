    ...
    try {
      ...
      const updatePromises = booksToUpdate.map(async ({ bookId, quantity, specificRemark }) => {
        ...
        if (historyDataForRoot) {
          ...
          throw new Error(`Gagal memproses buku ID ${bookId} (mungkin tidak ditemukan).`);
        }
      });
      ...
      if (error.code && error.message) {
        ...
        message.error(
          'Periksa kembali input form. Pastikan semua buku dan jumlah terisi dengan benar.'
        );
                            ...
                            >
                              {bukuList?.map(buku => (
                                <Option
                                  ...
                                  value={buku.id}
                                  disabled={
                                    selectedBookIdsInForm.has(buku.id) &&
                                    form.getFieldValue(['items', name, 'bookId']) !== buku.id
                                  }
                                >
