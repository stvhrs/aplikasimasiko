...
import { db } from '../api/firebase'; // Sesuaikan path ke file firebase.js kamu
...
  
  // PERUBAHAN: Simpan judul asli & buat salinan untuk diproses
  let judul = formatCellValue(namaBarangRaw); // <-- Ini judul asli, TIDAK AKAN DIUBAH
  let judul_untuk_diproses = formatCellValue(namaBarangRaw); // <-- Ini salinan untuk tebak mapel/penerbit
...
  // 3a. Ekstrak Penerbit (dari Nama Barang)
  const penerbitMatch = judul_untuk_diproses.match(/\(([^)]+)\)/);
  if (penerbitMatch && penerbitMatch[1]) {
    penerbit = penerbitMatch[1].trim().toLowerCase();
    judul_untuk_diproses = judul_untuk_diproses.replace(penerbitMatch[0], '').trim(); // Modifikasi salinan
  }

  // 3b. Cek Revisi (dari Nama Barang)
  const revisiMatch = judul_untuk_diproses.match(/\(\s*REVISI\s*\)/i);
  if (revisiMatch) {
    tahun = "revisi";
    judul_untuk_diproses = judul_untuk_diproses.replace(revisiMatch[0], '').trim(); // Modifikasi salinan
  }

  // 3c. Ekstrak Kelas (dari Group atau Departemen)
  if (groupStr === "UMUM") {
      kelas = "UMUM";
  } else {
...
  // 3d. Hapus sisa "KELAS..." dari Judul (hanya dari salinan)
  const sisaKelasDiJudulMatch = judul_untuk_diproses.match(/\.?\s*KELAS\s+([IVXLCDM\w\s]+)$/i);
  if (sisaKelasDiJudulMatch) {
      judul_untuk_diproses = judul_untuk_diproses.substring(0, sisaKelasDiJudulMatch.index).trim(); // Modifikasi salinan
  }
  judul_untuk_diproses = judul_untuk_diproses.replace(/[\s.]+$/, '').trim(); // Modifikasi salinan

  // 3e. Tebak Mapel (menggunakan 'judul_untuk_diproses')
...
  // 4a. Peruntukan (Siswa/Guru/UMUM) - dari Group
  let peruntukan;
  if (groupStr === "UMUM") {
      peruntukan = "UMUM"; 
  } else {
...
  // 4b. Spek (LKS/Buku) - dari Departemen
...
  // 4d. Stok - dari Qty (Key "Qty")
  const stokStr = formatCellValue(qtyRaw).replace(/,/g, ''); 
  const stok = parseInt(stokStr, 10) || 0;

  // --- 5. Buat Histori Stok Awal ---
  const historyRef = push(ref(db, 'buku/historiStok')); // Path 'buku/historiStok' hanya untuk generate key
  const historyKey = historyRef.key;

  const newHistoryEntry = {
    [historyKey]: {
      keterangan: "Cetakan dari Gubuk", // Sesuai permintaan
      perubahan: stok,                     // <-- DIUBAH: Sesuai stok awal
      stokSebelum: 0,                  // Stok awal selalu 0
      stokSesudah: stok,                // Stok dari data JSON
      timestamp: Date.now()            // Timestamp saat ini
    }
  };

  // --- 6. Buat Objek Data Final ---
  return {
    kode_buku: kodeBuku,
    judul: judul, // <-- Menggunakan 'judul' asli yang tidak dimodifikasi
    stok: stok,
    kelas: kelas,
...
    diskonCetak: 0,
    hargaCetakSpesial: 0,
    diskonCetakSpesial: 0,
    historiStok: newHistoryEntry, // Menggunakan data histori baru
    spekKertas: null,
  };
};
// =================================================================
...
  const [isLoading, setIsLoading] = useState(false); // Untuk parsing JSON
  const [isUploading, setIsUploading] = useState(false); // Untuk upload Firebase
  const [processedData, setProcessedData] = useState([]);
  const [jsonInput, setJsonInput] = useState(''); // State untuk teks JSON
...
  /**
   * Menyalin data JSON yang sudah diproses ke clipboard.
   */
